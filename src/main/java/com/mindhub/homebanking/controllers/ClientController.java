...
public class ClientController {
    ...
    public ResponseEntity<?> getClientById(@PathVariable Long id) {//@PathVariable long id: Vincula el parámetro de ruta {id} al parámetro id del método.
        //Captura el valor del {id} en la URL.
            Client client = clientRepository.findById(id).orElse(null); //Client client => Estoy declarando un objeto cliente de tipo Cliente que contendra los datos del cliente que coincida con el id especificado.
...
//        clientRepository.save(new Client(firstName, lastName, email));
    ...
    public ResponseEntity<?> updateClient(@PathVariable Long id, @RequestParam String firstName, @RequestParam String lastName, @RequestParam String email) {
        Client clienteAModificar = clientRepository.findById(id).orElse(null); //Estoy buscando el objeto que coicnida con el id que voy a pasar por parametro y para eso uso el metodo findById().//Este metodo viene de clienteRepository que a su vez viene de JpaRepository.
    ...
    @PatchMapping("/modifyClient")// Este método responderá a la solicitud de tipo PATCH en la ruta /api/clients/modificarCliente.
